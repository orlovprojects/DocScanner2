...
//   // Подтянуть документ при открытии/смене документа/смене выбранного контрагента (multi)
//   useEffect(() => {
//     if (
//       open &&
//       selected?.id &&
//       !String(selected.id).startsWith("temp-") &&
//       (prevDocId.current !== selected.id || isMulti) // пересчёт в multi при смене cp_key ниже
//     ) {
//       refreshDocument(selected.id);
//       prevDocId.current = selected.id;
//     }
...
//     }
//     // eslint-disable-next-line
//   }, [open, selected?.id, isMulti, selectedCpKey]);
...
//   // === PREVIEW labels для multi ===
//   const hasAnyCounterparty =
//     !!(selected?.buyer_id || selected?.buyer_vat_code || selected?.buyer_name) ||
//     !!(selected?.seller_id || selected?.seller_vat_code || selected?.seller_name);

//   const ppLabel = isMulti
//     ? (selected?.preview?.pirkimas_pardavimas_label || (hasAnyCounterparty ? "—" : "Pasirinkite kontrahentą"))
//     : (selected?.pirkimas_pardavimas === "pirkimas"
//         ? "Pirkimas"
//         : selected?.pirkimas_pardavimas === "pardavimas"
//           ? "Pardavimas"
...
//   const pvmLabel = isMulti
//     ? (selected?.preview?.pvm_kodas_label || (hasAnyCounterparty ? "—" : "Pasirinkite kontrahentą"))
//     : (selected?.pvm_kodas || "—");

//   const previewLinePvmById = (lineId) => {
//     const arr = selected?.preview?.line_items || [];
//     const hit = arr.find(li => String(li.id) === String(lineId));
//     return hit?.pvm_kodas_label || hit?.pvm_kodas || null;
//   };
...
//     // Сразу обновим локальный документ…
//     setSelected(res.data);
//     setDocs(prev => prev.map(d => sameId(d.id, selected.id) ? res.data : d));

//     // …и подтянем пересчитанное превью
//     if (isMulti) {
//       await refreshDocument(selected.id);
//     }
//   };
...
//               {/* Всегда показываем, но источник разный */}
//               <Typography gutterBottom>
//                 Pirkimas/pardavimas:&nbsp;<b>{ppLabel}</b>
//               </Typography>
...
//                 {/* В multi показываем только превьюшный PVM klasė */}
//                 <Typography>PVM klasė: <b>{pvmLabel}</b></Typography>
...
//                   <AccordionDetails>
//                     {lineItems.map((item, idx) => {
//                       const previewLinePvm = isMulti ? (previewLinePvmById(item.id) || "—") : (item.pvm_kodas || item.vat_class || "—");
//                       return (
//                         <Box
//                           key={item.id ?? `li-${idx}`}
//                           sx={{ mb: 2, p: 1, border: "1px solid #eee", borderRadius: 2, background: "#fff" }}
