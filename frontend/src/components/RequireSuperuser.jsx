...
const RequireSuperuser = ({ children, loginPath = "/prisijungti", forbiddenPath = "/403" }) => {
  const { isAuthenticated, loading, forceLogout } = useAuth();
  ...
  // 4) Nera teisiu / klaidos
  if (error === "forbidden") {
    return (
      <Box sx={{ maxWidth: 560, mx: "auto", mt: 10, p: 3, textAlign: "center" }}>
        <Typography variant="h5" gutterBottom>Prieiga uždrausta</Typography>
        <Typography color="text.secondary">
          Neturite teisių pasiekti šį puslapį.
        </Typography>
      </Box>
    );
  }
  ...
  if (error === "notfound") {
    return (
      <Box sx={{ maxWidth: 640, mx: "auto", mt: 10, p: 3, textAlign: "center" }}>
        <Typography variant="h6" gutterBottom>Endpoint nerastas (404)</Typography>
        <Typography color="text.secondary" sx={{ mb: 2 }}>
          Patikrinkite, ar teisingas URL ir serverio konfiguracija.
        </Typography>
  ...
  if (error) {
    return (
      <Box sx={{ maxWidth: 560, mx: "auto", mt: 10, p: 3, textAlign: "center" }}>
        <Typography variant="h6" color="error" gutterBottom>Klaida tikrinant teises</Typography>
  ...
  // 5) OK - leidziame toliau
  let prefetch = null;
  try { 
    prefetch = JSON.parse(sessionStorage.getItem("dashboardPrefetch") || "null"); 
  } catch {}
