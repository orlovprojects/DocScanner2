...
const InvoiceEditorPage = () => {
  ...
  useEffect(() => {
    ...
    if (activeId !== initialActiveIdRef.current) {
      initialActiveIdRef.current = activeId;
    ...
    (async () => {
      try {
        ...
        if (isNew && !duplicateFromId && settingsData) {
          ...
          const sp = settingsData.seller || {};
          const spCountry = findCountry(sp.country_iso);
          setForm((p) => ({
            ...p,
            seller_name: sp.name || '',
            seller_id: sp.company_code || '',
            seller_vat_code: sp.vat_code || '',
            seller_address: sp.address || '',
            seller_phone: sp.phone || '',
            seller_email: sp.email || '',
            seller_bank_name: sp.bank_name || '',
            seller_iban: sp.bank_account || '',
            seller_swift: sp.bank_swift || '',
            seller_type: sp.entity_type === 'iv' ? 'fizinis' : 'juridinis',
            seller_country: spCountry?.name || 'Lietuva',
            seller_country_iso: sp.country_iso || 'LT',
  ...
  const buildPayload = () => ({
    ...
    send_payment_reminders: form.invoice_type === 'kreditine' ? false : form.send_payment_reminders,
  ...
  const handleSave = async (andAction) => {
    ...
    try {
      ...
      if (!isRecurring && form.invoice_type !== 'kreditine' && paymentLink.enabled && paymentLink.provider && (newId || id)) {
        try {
          await api.post(
  ...
  const handleRecurringModeChange = (_, value) => {
    ...
    if (value === 'recurring' && form.invoice_type === 'kreditine') {
      // Периодическая кредитная не имеет смысла — сбрасываем на PVM SF
      u('invoice_type', 'pvm_saskaita');
      prevTypeRef.current = 'pvm_saskaita';
    }
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1400, mx: 'auto' }}>
      <Paper sx={{ p: { xs: 2, md: 3 }, borderRadius: 4, '& .MuiOutlinedInput-root': { backgroundColor: '#fff' } }}>
        ...
        {isEditable && form.invoice_type !== 'kreditine' && (
          <Box sx={{ mb: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, minWidth: 0, flexWrap: 'wrap' }}>
              <Switch
        ...
        {form.invoice_type === 'isankstine' && isEditable && (
          <Box sx={{ mb: 2 }}>
            ...
            {form.invoice_type !== 'kreditine' && (paymentLinksLocked ? (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, minWidth: 0, flexWrap: 'wrap' }}>
                <Switch
                  checked={false}
