...
import CompanyReplaceRulesSection from '../components/CompanyReplaceRulesSection';
...
export default function NustatymaiPage() {
  ...
  const [companyReplaceRules, setCompanyReplaceRules] = useState([]); 
  ...
  useEffect(() => {
    api.get("/profile/", { withCredentials: true }).then(({ data }) => {
      ...
      const crList = Array.isArray(data.company_replace_rules)
        ? data.company_replace_rules
        : data.company_replace_rules
        ? [data.company_replace_rules]
        : [];
      setCompanyReplaceRules(crList);
    });
  }, []);
  ...
  return (
    <Box sx={{ px: { xs: 2, md: 4 }, py: 4, maxWidth: 980, mx: "auto" }}>
      ...
      {/* 7. Kontrahentų keitimo taisyklės */}
      <CompanyReplaceRulesSection
        companyReplaceRules={companyReplaceRules}
        setCompanyReplaceRules={setCompanyReplaceRules}
      />
