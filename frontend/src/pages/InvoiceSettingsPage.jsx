...
const InvoiceSettingsPage = () => {
  ...
  // Seller (реквизиты активной фирмы — из CompanyProfile)
  ...
  const { activeId, refresh } = useCompanyProfiles();

  // ── Load (перезагружается при смене активной фирмы) ──
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
  ...
  const us = (field, value) => setSeller((p) => ({ ...p, [field]: value }));
  const isIV = seller?.entity_type === 'iv';
  const countryName = (iso) =>
    COUNTRY_OPTIONS.find((c) => c.code === iso)?.name || iso || '';
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // ── 1. Реквизиты продавца → CompanyProfile (PATCH) ──
      if (seller?.id) {
        const profilePayload = {
          ...
          iban: seller.bank_account || '',
        };
        // Идентичность editable только для юрлица (name/company_code);
        // для ИВ она производная (owner_name/iv_certificate_nr) — не шлём.
        if (!isIV) {
          profilePayload.name = seller.name || '';
          profilePayload.company_code = seller.company_code || '';
        }
        await api.patch(`/company-profiles/${seller.id}/`, profilePayload, {
          withCredentials: true,
        });
      }

      // ── 2. Логотип + дефолты + шаблоны → InvoiceSettings (PUT) ──
      ...
      // Перезагружаем settings (актуальный logo_url + seller) и контекст фирм
      const { data } = await invoicingApi.getSettings();
      setSettings(data);
      setSeller(data.seller || null);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 900, mx: 'auto' }}>
      <Paper sx={{ p: { xs: 2, md: 3 }, borderRadius: 4 }}>
        ...
        <Box sx={secSx}>
          <Typography sx={{ ...titleSx, color: P.primary }}>Pardavėjo duomenys</Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Išsaugojus pardavėjo rekvizitus, jie automatiškai užsipildys kuriant naujas sąskaitas
          </Typography>
            ...
            <Grid2 container spacing={1.5}>
              <Grid2 size={12}>
                <TextField fullWidth disabled
                  label={isIV ? 'Vardas Pavardė' : 'Pavadinimas'}
                  value={seller.name || ''} />
              </Grid2>
              <Grid2 size={{ xs: 12, sm: 6 }}>
                <TextField fullWidth disabled
                  label={isIV ? 'IV pažymėjimo nr.' : 'Įmonės kodas'}
                  value={seller.company_code || ''} />
              </Grid2>
              <Grid2 size={{ xs: 12, sm: 6 }}>
                <TextField fullWidth disabled label="PVM kodas"
                  value={seller.vat_code || ''} />
              </Grid2>
              <Grid2 size={12}>
                <TextField fullWidth label="Adresas"
                  value={seller.address || ''} onChange={(e) => us('address', e.target.value)} />
              </Grid2>
              <Grid2 size={12}>
                <TextField fullWidth disabled label="Šalis"
                  value={countryName(seller.country_iso)} />
              </Grid2>
              <Grid2 size={{ xs: 12, sm: 6 }}>
                <TextField fullWidth label="Telefonas"
                  value={seller.phone || ''} onChange={(e) => us('phone', e.target.value)} />
              </Grid2>
              <Grid2 size={{ xs: 12, sm: 6 }}>
                <TextField fullWidth label="El. paštas"
                  value={seller.email || ''} onChange={(e) => us('email', e.target.value)} />
              </Grid2>
              <Grid2 size={12}>
                <TextField fullWidth label="Banko pavadinimas"
                  value={seller.bank_name || ''} onChange={(e) => us('bank_name', e.target.value)} />
              </Grid2>
              <Grid2 size={{ xs: 12, sm: 8 }}>
                <TextField fullWidth label="IBAN"
                  value={seller.bank_account || ''} onChange={(e) => us('bank_account', e.target.value)} />
              </Grid2>
              <Grid2 size={{ xs: 12, sm: 4 }}>
                <TextField fullWidth label="SWIFT"
                  value={seller.bank_swift || ''} onChange={(e) => us('bank_swift', e.target.value)} />
              </Grid2>
            </Grid2>
          ) : (
            <Typography variant="body2" color="text.secondary">
              Nėra aktyvios įmonės. Sukurkite įmonę, kad matytumėte pardavėjo duomenis.
            </Typography>
          )}
        </Box>
