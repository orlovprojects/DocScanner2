...
export default function UploadPage() {
  ...
  // Export starting popup state (API exports only)
    ...
    try {
      ...
      if (err?.response?.data instanceof Blob) {
        try {
          ...
          alert("Eksportas nepavyko: " + (json.error || json.detail || "Klaida"));
        } catch {
          alert("Eksportas nepavyko: " + (err?.message || "Klaida"));
        }
      } else {
        const errMsg = err?.response?.data?.error || err?.message || "Klaida";
        alert("Eksportas nepavyko: " + errMsg);
      }
    }
  };
  ...
  return (
    <Box sx={{ p: isMobile ? 2 : 4 }}>
      ...
      {/* Export starting popup (shown while POST is in flight) */}
      <Dialog
        open={exportStarting}
        PaperProps={{
      ...
      <ExportStatusBar
        onExportComplete={async (session) => {
          // Рефрешим документы чтобы обновить api_status столбики
          await fetchDocs();
        }}
      />
...
//       if (isApiExport) {
//         // --- API export (Optimum, будущие) ---
//         // Отправляем как JSON, получаем JSON с session_id
//         const res = await api.post("/documents/export_xml/", payload, {
//           withCredentials: true,
//           // НЕ ставим responseType: "blob" !
//         });

//         // Запускаем polling в ExportStatusBar
//         if (ExportStatusBar._triggerPoll) {
//           ExportStatusBar._triggerPoll();
//         }
...
//       <ExportStatusBar
//         onExportComplete={async (session) => {
//           // Рефрешим документы чтобы обновить optimum_api_status столбики
//           await fetchDocs();
//         }}
//       />
