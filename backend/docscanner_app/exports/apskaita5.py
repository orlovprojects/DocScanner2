...
def _party_code_from_doc(
    ...
    *,
    ...
    Код стороны по приоритету:
      ...
      2) *_vat_code
      3) *_id_programoje
      4) fallback: NERAKODO#### (4 случайные цифры)
    """
    # 1) *_id
    sid = smart_str(getattr(doc, id_field, None) or '').strip()
    if sid:
        return sid
    # 2) *_vat_code
    svat = smart_str(getattr(doc, vat_field, None) or '').strip()
    if svat:
        return svat
    # 3) *_id_programoje
    sidp = smart_str(getattr(doc, id_programoje_field, None) or '').strip()
    if sidp:
        return sidp
    # 4) fallback
...
def _build_apskaita5_xml_for_documents(
    ...
    for doc in documents:
        ...
        # --- стороны (seller*/buyer*) — используем приоритет: *_id → *_vat_code → *_id_programoje ---
        seller_id_val = _party_code_from_doc(
            doc,
            id_field="seller_id",
        ...
        buyer_id_val = _party_code_from_doc(
            doc,
            id_field="buyer_id",
