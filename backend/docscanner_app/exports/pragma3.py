...
def _get_product_code(item=None, doc=None):
    """
    Получает код товара.
    Приоритет для строки: matched code/barcode -> original code/barcode ->
    doc.prekes_kodas -> doc.prekes_barkodas -> PREKE001.
    """
    if item:
        code = (
...
def _get_own_company_code(doc):
    """
    Получить код своей фирмы из документа.
    
    - При pirkimas: своя фирма = buyer
    - При pardavimas: своя фирма = seller
    """
    doc_type = _detect_document_type(doc)
    
    if doc_type == 1:  # pirkimas — своя фирма это buyer
        return (
            _s(getattr(doc, 'buyer_id', ''))
            or _s(getattr(doc, 'buyer_vat_code', ''))
        )
    else:  # pardavimas — своя фирма это seller
        return (
            _s(getattr(doc, 'seller_id', ''))
            or _s(getattr(doc, 'seller_vat_code', ''))
        )
...
def _parse_cp_key(cp_key):
    """
    Парсит cp_key в код компании.
    
    Форматы:
    - "id:304401940" → "304401940"
    - "LT304401940" → "LT304401940"
    - "304401940" → "304401940"
    """
    if not cp_key:
        return ""
...
def _get_extra_fields(user, program_key, own_company_code=None, doc=None):
    """
    Получить extra fields для экспорта.
    
    Приоритет для определения кода своей фирмы:
    1. own_company_code (из cp_key, переданного пользователем)
    2. Определяется из документа (buyer при pirkimas, seller при pardavimas)
    
    Приоритет для профиля:
    1. Профиль конкретной фирмы
    ...
    # Определяем код своей фирмы
    imones_kodas = _parse_cp_key(own_company_code)
    
    # Fallback: определяем из документа
    if not imones_kodas and doc:
        imones_kodas = _get_own_company_code(doc)
    
    result = get_extra_for_export(user, program_key, imones_kodas)
    
    logger.info("[PRAGMA:EXTRA] own_company_code='%s' imones_kodas='%s' profile_found=%s fields=%s",
                own_company_code, imones_kodas, bool(result), 
                {k: v for k, v in result.items() if v} if result else {})
...
def _export_documents(documents, doc_type_filter, user, company_id_map, own_company_code=None):
    """
    Экспорт документов в формат Pragma 3.2.
    
    Extra fields получаются на основе кода своей фирмы (own_company_code).
    """
    lines = []
    
    # Получаем extra fields один раз для всех документов (по своей фирме)
    pragma3_fields = _get_extra_fields(user, "pragma3", own_company_code)
    ...
    for doc in documents or []:
        ...
        # Sandėlis и Korespondencija из per-company pragma3_extra_fields
        if doc_type == 1:
        ...
        lines.append(line)
        logger.info("[PRAGMA:DOC] doc_id=%s type=%s company_id=%s warehouse=%s project=%s", 
                    doc_id, doc_type, company_numeric_id, warehouse, project_code)
...
def _export_document_items(documents, doc_type_filter, user, product_id_map, own_company_code=None):
    """
    Экспорт позиций документов в формат Pragma 3.2.
    ...
    for doc in documents or []:
        ...
        # Projekto kodas из per-company settings
        if doc_type == 1:
            project_code = _s(pragma3_fields.get('pirkimas_projektas', ''))
        else:
            project_code = _s(pragma3_fields.get('pardavimas_projektas', ''))
        ...
        # Получаем line_map от резолвера
        ...
        if has_items:
            ...
            logger.info("[PRAGMA:ITEMS] doc_id=%s exported %d line_items, project=%s", 
                        doc_id, line_items.count(), project_code)
        else:
            product_code = _get_product_code(None, doc)
            product_numeric_id = product_id_map.get(product_code, "")
            ...
            # PVM код для документа (sumiskai)
            ...
            logger.info("[PRAGMA:ITEMS] doc_id=%s exported as single line, project=%s", 
                        doc_id, project_code)
...
def export_to_pragma_full(documents, user=None, include_reference_data=True, own_company_code=None):
    """
    ...
    Extra fields получаются на основе кода своей фирмы (own_company_code),
    который передаётся из view (cp_key).
    ...
    Args:
        ...
        own_company_code: код своей фирмы (из cp_key) для поиска профиля extra_fields
    ...
    logger.info("[PRAGMA:EXPORT] starting export: docs=%d, own_company_code=%s", 
    ...
    #    Extra fields получаются по own_company_code
    if has_pirkimai:
        result['pirkimai'] = _export_documents(
            documents, doc_type_filter=1, user=user, 
            company_id_map=company_id_map, own_company_code=own_company_code
        )
        result['pirkimai_det'] = _export_document_items(
            documents, doc_type_filter=1, user=user, 
            product_id_map=product_id_map, own_company_code=own_company_code
        )
    
    if has_pardavimai:
        result['pardavimai'] = _export_documents(
            documents, doc_type_filter=2, user=user, 
            company_id_map=company_id_map, own_company_code=own_company_code
        )
        result['pardavimai_det'] = _export_document_items(
            documents, doc_type_filter=2, user=user, 
            product_id_map=product_id_map, own_company_code=own_company_code
        )
