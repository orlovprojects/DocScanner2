...
def _use_matched_catalog(item) -> bool:
    ...
    if _get_attr(item, "catalog_match_user_override", False):
        return False

    matched_code = _s(_get_attr(item, "matched_prekes_kodas", ""))
...
def _resolved_field(item, field_name: str) -> str:
    ...
    if _use_matched_catalog(item):
        matched_value = _s(
            _get_attr(item, f"matched_{field_name}", "")
        )
        if matched_value:
            return matched_value

    return _s(_get_attr(item, field_name, ""))
...
def _get_barcode(item=None, doc=None) -> str:
    if item is not None:
        barcode = _resolved_field(item, "prekes_barkodas")
        if barcode:
            return barcode
...
def _is_merge_vat(customuser) -> bool:
    """
    Проверяет флаг merge_vat из user.extra_settings.
    При True PVM включается в цену и отдельно не экспортируется.
    """
    if customuser is None:
        return False

    extra_settings = _get_attr(customuser, "extra_settings", None)
...
def _resolve_type_group_product(item=None, doc=None) -> tuple[str, str, bool]:
    v = _resolved_field(item, "preke_paslauga") if item is not None else ""
    if not v and doc is not None:
...
# =========================================================
def _get_vat_tariff(doc, item=None, merge_vat: bool = False) -> str:
    if merge_vat:
        return "0"
...
def _compute_line_discounts(
    ...
    for item in line_items:
        ...
        if merge_vat:
            vat_raw = _get_attr(item, "vat", None)
    ...
    for i, line_total in enumerate(items_data):
        if i == len(items_data) - 1:
            ...
            part = (disc * line_total / total).quantize(
                Decimal("0.01"),
                rounding=ROUND_HALF_UP,
            )
            result[i] = part
...
def build_insert_article_xml(
    *,
    ...
    merge_vat = _is_merge_vat(customuser)
    ...
    ) or _s(_get_attr(doc, "prekes_pavadinimas", "")) or "Prekė"
    unit = (
        _resolved_field(item, "unit")
        if item is not None
        else ""
    ) or _s(_get_attr(doc, "unit", "")) or "vnt."
    msr = _normalize_unit(unit)
    currency = (_s(_get_attr(doc, "currency", "")) or "EUR").upper()
    vat_tariff = "0" if merge_vat else _to_decimal_str(
...
def build_insert_invoice_xml(
    *,
    ...
    separate_vat = bool(_get_attr(doc, "separate_vat", False))
    scan_type = _s(_get_attr(doc, "scan_type", "")).lower()
    is_sumiskai_separate = (
        not merge_vat
        and separate_vat
        and scan_type == "sumiskai"
    )
    ...
    if has_items:
        ...
        line_discounts = _compute_line_discounts(
            doc,
            items_list,
            merge_vat=merge_vat,
        )
        ...
        for i, item in enumerate(items_list):
            ...
            if merge_vat:
                line_total_before_discount = price * qty
                vat_raw = _get_attr(item, "vat", None)
                ...
                new_unit_price = (line_total / qty).quantize(
                    Decimal("0.01"),
                    rounding=ROUND_HALF_UP,
                ) if qty else line_total
        ...
        vat_codes = (
            []
            if merge_vat
            else _build_vat_codes(doc, items_list, line_map)
        )
    else:
        ...
        vat_amount = _safe_D(_get_attr(doc, "vat_amount", 0) or 0)
        discount = _safe_D(
            _get_attr(doc, "invoice_discount_wo_vat", 0) or 0
        )
        vat_tariff = _get_vat_tariff(
            doc,
            item=None,
        ...
        if merge_vat:
            line_total = amount_wo + vat_amount - discount
        ...
        vat_codes = (
            []
            if merge_vat
            else _build_vat_codes(doc, None, line_map)
        )
