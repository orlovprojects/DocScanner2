...
def _get_item_identity(doc, it=None):
    """
    Возвращает (name, code, barcode) с приоритетом matched item -> original item -> doc.
    """
    if it is not None:
        name = (
            _resolved_field(it, "prekes_pavadinimas")
            or _s(getattr(doc, "prekes_pavadinimas", ""))
        )
        code = (
            _resolved_field(it, "prekes_kodas")
            or _s(getattr(doc, "prekes_kodas", ""))
        )
        barcode = (
            _resolved_field(it, "prekes_barkodas")
