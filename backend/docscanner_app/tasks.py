    ...
    try:
        ...
        if _pre_checked_dup:
            ...
            if similarity_percent >= 95:
                ...
                try:
                    from .utils.gemini import ask_gemini_lite_with_model_fallback
                    resp = ask_gemini_lite_with_model_fallback(
...
def monitor_stuck_sessions():
    ...
    for s in stuck_processing:
        ...
        if pending_count == 0:
            ...
            if recent_activity:
                # Воркеры живы, просто очередь длинная — не трогаем
                wait_min = int((now - s.updated_at).total_seconds() // 60)
                if wait_min > 60:
                    alerts.append(
                        f"⏳ Session <code>{str(s.id)[:8]}</code> waiting {wait_min}m\n"
                        f"   {pending_count} pending, workers busy with other sessions"
                    )
            else:
                # Воркеры мертвы 30+ мин — reject зависшие доки
                ...
                for d in stale_docs:
                    try:
                        d.status = "rejected"
                        d.error_message = "Automatiškai atšaukta: dokumentas užstrigo apdorojimo eilėje"
                        d.save(update_fields=["status", "error_message"])
                        _settle_and_finish_if_session(d)
                        rejected += 1
                    except Exception as e:
                        logger.error("[WATCHDOG] Reject stuck doc %s: %s", d.id, e)

                if rejected:
                    fixes.append(
                        f"🧟 Session <code>{str(s.id)[:8]}</code>: "
                        f"rejected {rejected}/{pending_count} stuck docs (no worker activity)"
                    )

    # ─── 2b. Документы completed/exported но не settled ───
    unsettled_docs = ScannedDocument.objects.filter(
        status__in=["completed", "exported"],
        counted_in_session=False,
        upload_session__isnull=False,
        uploaded_at__lt=now - timedelta(minutes=15),
    )
    for d in unsettled_docs:
        try:
            _settle_and_finish_if_session(d)
            fixes.append(f"🔗 Doc {d.id} settled (completed but unsettled)")
        except Exception as e:
            logger.error("[WATCHDOG] Settle unsettled doc %s: %s", d.id, e)
    ...
    orphan_fixed = 0
    for d in orphan_docs[:50]:
        try:
            d.status = "rejected"
            d.error_message = "Automatiškai atšaukta: dokumentas liko be aktyvios sesijos"
            d.save(update_fields=["status", "error_message"])
            _settle_and_finish_if_session(d)
            orphan_fixed += 1
        except Exception as e:
            logger.error("[WATCHDOG] Fix orphan doc %s: %s", d.id, e)
    if orphan_fixed:
        fixes.append(f"📄 Fixed {orphan_fixed} orphan docs → rejected")
    ...
    # ─── 7. blocked > 2 часа → failed ───
    stuck_blocked = UploadSession.objects.filter(
        stage="blocked",
        updated_at__lt=now - timedelta(hours=2),
    )
    for s in stuck_blocked:
        try:
            with transaction.atomic():
                ss = UploadSession.objects.select_for_update().get(id=s.id)
                if ss.stage != "blocked":
                ...
                ss.stage = "failed"
                ss.finished_at = now
                ss.error_message = "Automatiškai atšaukta: nepapildyti kreditai per 2 val."
                ss.save(update_fields=[
            ...
            fixes.append(
                f"🚫 Blocked session <code>{str(s.id)[:8]}</code> → failed (>2h)"
            )
        except Exception as e:
            alerts.append(
                f"🚫 Blocked session <code>{str(s.id)[:8]}</code> fix failed: {str(e)[:200]}"
            )
...
@shared_task(bind=True, soft_time_limit=300, time_limit=330, reject_on_worker_lost=True)
...
@shared_task(bind=True, soft_time_limit=180, time_limit=210, reject_on_worker_lost=True)
