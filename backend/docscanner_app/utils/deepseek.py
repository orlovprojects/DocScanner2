import os
import time
import openai  # используем для совместимого RateLimitError
...
if not DEEPSEEK_API_KEY:
    raise RuntimeError("Не найден DEEPSEEK_API_KEY в переменных окружения.")
...
# =========================
# 2) ПРОМПТЫ ДЛЯ DEEPSEEK
#    <<< ВСТАВЬ СЮДА СВОЙ DEFAULT_PROMPT >>>
# =========================
DEEPSEEK_DEFAULT_PROMPT = """You will receive raw OCR text from one or more scanned financial documents, which may include invoices, receipts, or similar forms.
...
# =========================
#    <<< ВСТАВЬ СЮДА СВОЙ DETAILED_PROMPT >>>
# =========================
...
# 3) ОСНОВНЫЕ ФУНКЦИИ
# =========================
def ask_deepseek(text: str, prompt: str, model: str = "deepseek-chat", temperature: float = 0.3, max_tokens: int = 5048) -> str:
    """
    Делает один запрос к DeepSeek (chat.completions.create) с твоим промптом и текстом OCR.
    По умолчанию модель 'deepseek-chat'. Для R1 можно поставить 'deepseek-reasoner'.
    """
    full_prompt = prompt + "\n\n" + text

    response = deepseek_client.chat.completions.create(
        model=model,  # 'deepseek-chat' или 'deepseek-reasoner' (R1)
        messages=[
            {
                "role": "system",
                "content": (
                    "You are a helpful assistant that extracts and structures data from documents. "
                    "The input text contains positional coordinates for each text block in the format "
                    "[x1,y1; x2,y2; x3,y3; x4,y4]. Use these coordinates to accurately identify and extract fields, "
                    "especially when similar fields appear more than once."
                )
            },
...
def ask_deepseek_with_retry(
    ...
    """
    Повторяет запрос при 429/Rate Limit до max_retries раз.
    """
    last_exc = None
    for attempt in range(max_retries + 1):
        try:
            ...
            print(f"[DeepSeek] Rate limit (429). Попытка {attempt + 1}/{max_retries + 1}. Ждём {wait_seconds} сек...")
            last_exc = e
            ...
            if "rate limit" in msg or "429" in msg:
                print(f"[DeepSeek] Похоже rate limit. Попытка {attempt + 1}/{max_retries + 1}. Ждём {wait_seconds} сек...")
                last_exc = e
                if attempt < max_retries:
                    time.sleep(wait_seconds)
            else:
