...
PREVIEW_TARGET_SIZE = 100_000
PREVIEW_MAX_SIZE = 200_000
...
def _save_as_jpeg_bytes(img: Image.Image, quality: int) -> bytes:
    buf = io.BytesIO()
    # конвертация в RGB + белый фон для прозрачности
...
def optimize_preview_image_to_jpeg_bytes(input_path: str,
                                         target_size: int = PREVIEW_TARGET_SIZE,
                                         max_size: int = PREVIEW_MAX_SIZE) -> Tuple[bytes, int]:
    ...
    logger.info(f"[PREVIEW] Original size: {img.width}x{img.height}")
    ...
    while quality >= PREVIEW_MIN_QUALITY:
        data = _save_as_jpeg_bytes(img_resized, quality)
        size = len(data)
        logger.debug(f"[PREVIEW] Quality {quality}: {size} bytes")
        if size <= max_size:
            best = (data, quality)
            if size <= target_size:
                logger.info(f"[PREVIEW] Target achieved at Q={quality}, size={size}")
                break
        # шаг понижений
    ...
    if best is None:
        data = _save_as_jpeg_bytes(img_resized, PREVIEW_MIN_QUALITY)
        best = (data, PREVIEW_MIN_QUALITY)

    data, q = best
    return data, q

def optimize_preview_for_document(doc) -> bool:
    """
    Делает JPEG-превью, сохраняет как НОВЫЙ *.jpg файл,
    удаляет старый файл и обновляет doc.file.
    """
    if not getattr(doc, 'file', None):
        logger.warning(f"[PREVIEW] Document {doc.id} has no file")
        return False

    abs_old_path = doc.file.path
    if not os.path.exists(abs_old_path):
        logger.warning(f"[PREVIEW] File not found: {abs_old_path}")
        return False

    # 1) Сжать в JPEG (байты)
    data, quality = optimize_preview_image_to_jpeg_bytes(abs_old_path)

    # 2) Сформировать новое имя с .jpg (в той же папке uploads/USER/uuid.jpg)
    rel_old = doc.file.name  # относительный путь от MEDIA_ROOT
    base, _old_ext = os.path.splitext(rel_old)
    new_rel = base + ".jpg"
    # на всякий caso: если у base уже заканчивается на .png/.jpg двоично — норм, os.path.splitext корректно отделил.

    # 3) Сохранить новый файл через storage
    # Убедимся, что директория существует
    dir_rel = os.path.dirname(new_rel)
    dir_abs = os.path.join(settings.MEDIA_ROOT, dir_rel)
    os.makedirs(dir_abs, exist_ok=True)

    # Если файл уже существует, сменим имя (очень маловероятно)
    candidate = new_rel
    i = 1
    ...
    try:
        if default_storage.exists(rel_old) and rel_old != new_rel:
            default_storage.delete(rel_old)
    except Exception as e:
        logger.warning(f"[PREVIEW] Could not delete old file {rel_old}: {e}")

    # 5) Обновить модель
    doc.file.name = new_rel
    # Если есть поле для mime/type — можно обновить, например: doc.mime_type = 'image/jpeg'
    doc.save(update_fields=['file'])

    logger.info(f"[PREVIEW] Document {doc.id} saved as JPEG: {new_rel} (quality={quality}, bytes={len(data)})")
    return True
...
# PREVIEW_MAX_HEIGHT = 3000     # максимальная высота для preview
# PREVIEW_TARGET_SIZE = 100_000 # целевой размер ~100KB
# PREVIEW_MAX_SIZE = 200_000    # абсолютный максимум 150KB
