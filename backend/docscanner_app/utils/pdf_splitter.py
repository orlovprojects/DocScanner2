...
# Каждый батч кроме первого начинается с последнего документа предыдущего
# батча. Модель не знает про батчи — она просто видит связный кусок PDF
# и нумерует страницы с 1. Перекрытие потом отбрасываем.
MAX_OVERLAP_PAGES = 10
OVERLAP_TAIL_PAGES = 3
...
def _classify_batch(
    ...
    """
    Отправляет batch страниц в Gemini.

    Модель не знает про батчи: она видит связный фрагмент и нумерует
    страницы с 1. Смещение в оригинальные номера делаем здесь.
    page_start/page_end: 0-based, полуинтервал.
    """
    pages = list(range(page_start, page_end))
    count = len(pages)
    pdf_bytes = extract_pages_as_pdf_bytes(pdf_path, pages)

    prompt = SPLIT_PROMPT_TEMPLATE.format(count=count)
    ...
    def _to_original(raw_pages) -> list[int]:
        out = []
        seen = set()
        for p in raw_pages or []:
            try:
                ...
                continue
            if rel < 1 or rel > count:
                continue
            orig = page_start + rel - 1
            if orig not in seen:
                seen.add(orig)
                out.append(orig)
        return sorted(out)
    ...
    for item in docs:
        item_pages = _to_original(item.get("pages"))
    ...
    for item in result.get("unassigned_pages") or []:
        if isinstance(item, dict):
            raw_page = item.get("page")
            kind = (item.get("kind") or "").strip().lower()
        else:
        ...
        orig = _to_original([raw_page])
        if not orig or orig[0] in seen_un:
            continue
        ...
        seen_un.add(orig[0])
        unassigned.append({"page": orig[0], "kind": kind})

    unassigned.sort(key=lambda x: x["page"])

    result["documents"] = normalized_docs
    result["unassigned_pages"] = unassigned

    logger.info(
        "[PDF-SPLIT] Batch pages %d-%d (%d pages): groups=%d unassigned=%s",
        page_start + 1,
        page_end,
        count,
        len(normalized_docs),
        [(u["page"] + 1, u["kind"]) for u in unassigned],
    )
...
def _pick_overlap_start(groups: list[dict], page_start: int) -> int:
    """
    Возвращает 0-based страницу, с которой начать следующий батч,
    чтобы захватить последний документ предыдущего батча.

    Документ до MAX_OVERLAP_PAGES берём целиком, длиннее — только
    последние OVERLAP_TAIL_PAGES страниц.
    """
    real = [
        g for g in groups
        if g.get("pages") and not g.get("unassigned_kind")
    ]

    if not real:
        return page_start

    last = max(real, key=lambda g: max(g["pages"]))
    pages = sorted(last["pages"])

    if max(pages) != page_start - 1:
        # Последний документ не примыкает к границе — перекрывать нечем
        return page_start

    if len(pages) <= MAX_OVERLAP_PAGES:
        return pages[0]

    return pages[-OVERLAP_TAIL_PAGES]


def _drop_overlap_groups(
    ...
    """
    Отбрасывает из ответа модели то, что относится к зоне перекрытия
    (страницы overlap_start..overlap_end-1), которая уже разобрана
    предыдущим батчем.

    Группа, которая ЧАСТИЧНО выходит за перекрытие, означает, что модель
    считает документ продолжающимся дальше — такую оставляем, а старую
    версию в known_groups подрежем в вызывающем коде.
    """
    if overlap_start >= overlap_end:
        return new_groups, new_unassigned

    overlap = set(range(overlap_start, overlap_end))
    ...
    for g in new_groups:
        ...
        inside = pages & overlap
        outside = pages - overlap
        ...
        if not outside:
            # Группа целиком в перекрытии — уже знаем её.
            old = known_by_page.get(min(inside))
            old_num = _norm_code(old.get("number")) if old else ""
            new_num = _norm_code(g.get("number"))

            if old_num and new_num and old_num != new_num:
                logger.warning(
                    "[PDF-SPLIT] Overlap number mismatch on pages %s: known=%s new=%s (keeping known)",
                    [p + 1 for p in sorted(inside)], old_num, new_num,
                )
            continue

        # Документ продолжается за перекрытие — берём новую версию целиком.
        logger.info(
            "[PDF-SPLIT] Overlap group extends past boundary, taking new version: %s",
            [p + 1 for p in sorted(pages)],
        )
        kept.append(g)

    kept_un = [u for u in new_unassigned if u["page"] not in overlap]
...
def _classify_range_with_fallback(
    ...
    if size > MIN_BATCH_SIZE and depth < MAX_BATCH_RETRY_DEPTH:
        ...
        # Вторая половина тоже должна видеть хвост первой, иначе она
        # остаётся без контекста — ровно та проблема, что и у батчей.
        right_start = _pick_overlap_start(left_groups, mid) if left_groups else mid

        time.sleep(BATCH_PAUSE_SECONDS)
        right_groups, right_un = _classify_range_with_fallback(
            pdf_path, right_start, page_end, total_pages, False, depth + 1,
        )

        if right_start < mid:
            right_groups, right_un = _drop_overlap_groups(
                right_groups, right_un, right_start, mid, left_groups,
            )

            taken = set()
            for g in right_groups:
                taken.update(g.get("pages") or [])

            if taken:
                merged = []
                for g in left_groups:
                    rest = [p for p in (g.get("pages") or []) if p not in taken]
                    if not rest:
                        continue
                    if len(rest) != len(g.get("pages") or []):
                        g = dict(g)
                        g["pages"] = rest
                    merged.append(g)
                left_groups = merged
                left_un = [u for u in left_un if u["page"] not in taken]
...
def classify_pdf_pages(pdf_path: str, total_pages: int) -> list[dict]:
    ...
    Pipeline:
    1. Batch pass по 10 страниц (с перекрытием)
    2. Draft page map
    ...
    while next_page < total_pages:
        ...
        # Захватываем последний документ предыдущего батча, чтобы модель
        # видела, где он кончился, и не гадала про первую страницу.
        overlap_start = _pick_overlap_start(groups, next_page) if groups else next_page
        page_start = overlap_start
        page_end = min(page_start + batch_size, total_pages)

        if page_end <= next_page:
            break

        logger.info(
            "[PDF-SPLIT] Batch %d: pages %d-%d of %d (batch_size=%d, overlap=%d)",
            batch_index,
            page_start + 1,
            page_end,
            total_pages,
        ...
        if page_start < next_page:
            batch_groups, batch_unassigned = _drop_overlap_groups(
                batch_groups,
                batch_unassigned,
                page_start,
                next_page,
            ...
            # Если новая версия документа залезла в перекрытие,
            # убираем эти страницы из старых групп, чтобы не было дублей.
            taken = set()
            ...
            if taken:
                ...
                for g in groups:
                    ...
                    merged.append(g)
                groups = merged
                all_unassigned = [u for u in all_unassigned if u["page"] not in taken]

        groups.extend(batch_groups)
        all_unassigned.extend(batch_unassigned)

        stats["batches"] += 1
        stats["overlap_pages"] += max(next_page - page_start, 0)
    ...
    unassigned_final = [g for g in final_groups if g.get("unassigned_kind")]
    fallback_final = [
        g for g in final_groups
        if (g.get("method") or "").startswith("batch_fallback")
    ]

    logger.info(
        "[PDF-SPLIT] STATS pages=%d batches=%d overlap_pages=%d docs=%d "
        "unassigned=%d fallback=%d verifications=%d merges=%d",
        total_pages,
        stats["batches"],
        stats["overlap_pages"],
        len(final_groups) - len(unassigned_final),
        len(unassigned_final),
        len(fallback_final),
        stats["verifications"],
        stats["merges"],
    )
...
SPLIT_PROMPT_TEMPLATE = """You are given a PDF fragment with {count} pages.
Number the pages 1..{count} exactly as they appear in this fragment.
It may contain multiple accounting documents (VAT invoices, invoices, credit notes, receipts, etc.).

Group the pages by document.
...
Every page from 1 to {count} MUST appear EXACTLY ONCE in your answer,
...
of the previous sheet. An empty back side belongs to the document on the sheet
before it — include it in that document's "pages", do NOT make it a separate
document and do NOT put it into "unassigned_pages".
Decide the pattern once and keep it consistent for the whole fragment.

Put a page into "unassigned_pages" ONLY if it belongs to no document at all,
and set "kind":
- "blank": an empty sheet that is not the back side of any invoice
- "separator": a divider sheet, barcode sheet, or service page
- "fragment": the page clearly belongs to an accounting document whose
  beginning and end are NOT present in this fragment
...
Field rules:
- pages: page numbers within THIS fragment, from 1 to {count}
- complete: false only if the last document is cut off at page {count}
...
- unassigned_pages: page numbers between 1 and {count} that belong to no document, each with a kind. Use an empty array if every page belongs to a document
"""
