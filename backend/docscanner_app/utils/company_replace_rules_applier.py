...
def _matches_side(rule, side_data):
    ...
    # Kodas
    match_kodas_op = (rule.get("match_kodas_op") or "").strip().lower()
    match_kodas = _norm(rule.get("match_kodas"))
    side_kodas = _norm(side_data.get("id"))

    # Backward compatibility со старыми правилами
    if not match_kodas_op:
        match_kodas_op = "eq" if match_kodas else "any"

    if match_kodas_op != "any":
        conditions_checked += 1

        if match_kodas_op == "eq":
            if not match_kodas or side_kodas != match_kodas:
                return False

        elif match_kodas_op == "empty":
            if side_kodas:
                return False

        elif match_kodas_op == "not_empty":
            if not side_kodas:
                return False

    # PVM kodas
    match_pvm_op = (rule.get("match_pvm_kodas_op") or "").strip().lower()
    match_pvm = _norm(rule.get("match_pvm_kodas"))
    side_pvm = _norm(side_data.get("vat_code"))

    # Backward compatibility со старыми правилами
    if not match_pvm_op:
        match_pvm_op = "eq" if match_pvm else "any"

    if match_pvm_op != "any":
        conditions_checked += 1

        if match_pvm_op == "eq":
            if not match_pvm or side_pvm != match_pvm:
                return False

        elif match_pvm_op == "empty":
            if side_pvm:
                return False

        elif match_pvm_op == "not_empty":
            if not side_pvm:
                return False
