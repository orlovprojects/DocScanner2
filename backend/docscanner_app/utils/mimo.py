...
# =========================
# 1) КЛИЕНТ MIMO (Xiaomi) + таймаут
# =========================

MIMO_API_KEY = os.getenv("MIMO_API_KEY")
if not MIMO_API_KEY:
    raise RuntimeError("Не найден MIMO_API_KEY в переменных окружения.")

MIMO_API_URL = os.getenv("MIMO_API_URL", "https://api.xiaomimimo.com/v1")
MIMO_TIMEOUT_SECONDS = float(os.getenv("MIMO_TIMEOUT_SECONDS", "300"))  # 5 минут по умолчанию
...
def _get_client() -> OpenAI:
    ...
    if _mimo_client is None:
        _mimo_client = OpenAI(
            ...
            base_url=MIMO_API_URL,
        )
    return _mimo_client


# =========================
# 2) ПРОМПТЫ (такие же как в grok.py)
# =========================

MIMO_DEFAULT_PROMPT = """You will receive raw OCR text from one or more scanned financial documents, which may include invoices, receipts, or similar forms.
...
# =========================
# 3) НИЗКОУРОВНЕВЫЙ ВЫЗОВ MiMo Chat Completions
# =========================
...
def ask_mimo(
    *,
    ...
    """
    Один запрос к MiMo API.
    prompt уходит как system, text как user.
    """
    log = logger or LOGGER
    eff_timeout = float(timeout_seconds if timeout_seconds is not None else MIMO_TIMEOUT_SECONDS)
    ...
    log.info(
        "[MiMo] Request start model=%s len_text=%d len_prompt=%d timeout=%ss thinking=%s",
        model, len(text or ""), len(prompt or ""), eff_timeout, enable_thinking
    )
    ...
    # Настройка thinking
    thinking_config = {"type": "enabled"} if enable_thinking else {"type": "disabled"}
    ...
    try:
        ...
        log.info("[MiMo] OK model=%s len=%d elapsed=%.2fs preview=%r", model, len(result), elapsed, preview)
        return result
        
    except Exception as e:
        elapsed = time.perf_counter() - t0
        log.warning("[MiMo] Error after %.2fs: %s", elapsed, e)
        raise


def ask_mimo_with_retry(
    *,
    text: str,
    prompt: str,
    model: str = "mimo-v2-flash",
    ...
    for attempt in range(max_retries + 1):
        log.info("[MiMo] Attempt %d/%d model=%s timeout=%ss", attempt + 1, max_retries + 1, model, timeout_seconds or MIMO_TIMEOUT_SECONDS)
        ...
        try:
            ...
            retryable = any(k in err_str for k in [
                ...
                "connection", "502", "503", "504", "429"
            ])
            
            log.warning(
                "[MiMo] Error retryable=%s attempt=%d/%d elapsed=%.2fs err=%s",
                retryable, attempt + 1, max_retries + 1, elapsed_attempt, e,
                exc_info=True,
            )
            last_exc = e
    ...
    raise last_exc or RuntimeError("Unknown MiMo retry failure")
...
def repair_truncated_json_with_mimo(
    *,
    broken_json: str,
    glued_raw_text: str,
    model: str = "mimo-v2-flash",
...
# =========================
# 6) FOLLOW-UP: truncated_json=true -> полный JSON без лимита
# =========================

def build_truncated_followup_prompt(glued_raw_text: str, previous_json: str) -> Tuple[str, str]:
    prompt = (MIMO_TRUNCATED_TO_FULL_PROMPT or "").strip()
    if not prompt:
        prompt = (
...
def request_full_json_with_mimo(
    *,
    glued_raw_text: str,
    previous_json: str,
    model: str = "mimo-v2-flash",
    ...
    if logger:
        logger.info("[MiMo][FULL] prompt_len=%s text_len=%s prompt_head=%r",
