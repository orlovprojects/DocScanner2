...
def _build_line_items(inv, s):
    ...
    for idx, li in enumerate(lines, 1):
        raw_qty = _parse_num(li.quantity)
        raw_price = _parse_num(li.price)
        raw_discount = _parse_num(li.discount_wo_vat or 0)
        raw_subtotal = _parse_num(li.subtotal or 0)

        vat_pct = (
            _parse_num(li.vat_percent)
            if li.vat_percent is not None
            else (
                _parse_num(inv.vat_percent or 21)
                if is_pvm
                else Decimal("0")
            )
        )
        ...
        if is_credit:
            # Kreditinėje kiekis ir sumos rodomi su minusu.
            # Kaina ir PVM procentas lieka teigiami.
            qty = -abs(raw_qty)
            price = abs(raw_price)
            discount = -abs(raw_discount)
            vat_pct = abs(vat_pct)

            # Naudojame backend išsaugotą subtotal.
            net = -abs(raw_subtotal)
        else:
            qty = raw_qty
            price = raw_price
        ...
        if has_discount:
            row.append(
                Paragraph(
                    _fmt(discount) if abs(discount) > 0 else "",
                    s["td_right"],
                )
            )
...
def _build_totals(inv, s):
    """
    PDF naudoja tas pačias Invoice galutines sumas kaip DK:
    - amount_wo_vat
    - vat_amount
    - amount_with_vat
    """
    ...
    lines = list(
        inv.line_items.order_by("sort_order", "id")
    )
    ...
    def money(value):
        return _parse_num(value).quantize(
            MONEY,
            rounding=ROUND_HALF_UP,
        )

    # Eilučių subtotal prieš bendrą dokumento nuolaidą.
    sum_net_abs = sum(
        (
            abs(_parse_num(line.subtotal or 0))
            for line in lines
        ),
        ZERO,
    )
    ...
    invoice_discount_abs = min(
        abs(
            _parse_num(
                inv.invoice_discount_wo_vat or 0
            )
        ),
        sum_net_abs,
    )
    ...
    inv_discount = (
        ...
        if is_credit
        else invoice_discount_abs
    )

    # Vienintelis galutinių sumų šaltinis.
    base = money(inv.amount_wo_vat or 0)
    vat_total = money(inv.vat_amount or 0)
    grand = money(inv.amount_with_vat or 0)

    # PVM grupių svoriai pagal išsaugotus subtotal.
    vat_groups = {}

    for line in lines:
        line_net_abs = abs(
            _parse_num(line.subtotal or 0)
        )

        vat_percent = (
            abs(_parse_num(line.vat_percent))
            if line.vat_percent is not None
            else (
                abs(_parse_num(inv.vat_percent or 21))
                if is_pvm
                else ZERO
            )
        )
        ...
        vat_groups[vat_percent] += line_net_abs

    breakdown = []
    rates = sorted(vat_groups.keys(), reverse=True)
    ...
    target_base_abs = abs(base)
    allocated_base = ZERO

    # Paskirstome galutinę amount_wo_vat tarp PVM grupių.
    for index, rate in enumerate(rates):
        if index == len(rates) - 1:
            discounted_abs = (
                target_base_abs - allocated_base
            )
        elif total_weight > ZERO:
            discounted_abs = money(
                target_base_abs *
                vat_groups[rate] /
                total_weight
            )
            allocated_base += discounted_abs
        else:
            discounted_abs = ZERO
    ...
    # Paskutinė apmokestinama grupė gauna PVM centų likutį,
    # kad breakdown sutaptų su Invoice.vat_amount.
    taxable_indexes = [
        index
        for index, group in enumerate(breakdown)
        if group["rate"] > 0
    ]

    if taxable_indexes:
        calculated_vat_abs = sum(
            (
                abs(group["vat"])
        ...
        if vat_residual != ZERO:
            last_index = taxable_indexes[-1]

            corrected_vat_abs = (
                abs(breakdown[last_index]["vat"]) +
                vat_residual
            )

            breakdown[last_index]["vat"] = (
                -corrected_vat_abs
                if is_credit
                else corrected_vat_abs
            )

    multi_vat = len(breakdown) > 1
    rows = []

    if abs(inv_discount) > ZERO:
        rows.append((
            "normal",
            "Tarpinė suma:",
            f"{_fmt(sum_net)} {sym}",
    ...
    if is_pvm:
        ...
        if multi_vat:
            for group in breakdown:
                rate = (
                    int(group["rate"])
                    if group["rate"] == int(group["rate"])
                    else group["rate"]
                )

                rows.append((
                    "indent",
                    f"Apmokestinama PVM {rate}%:",
                    f"{_fmt(group['net'])} {sym}",
                ))
            ...
            for group in breakdown:
                if group["rate"] > 0:
                    ...
                    rows.append((
                        "normal",
                        f"PVM {rate}%:",
                        f"{_fmt(group['vat'])} {sym}",
                    ))
        else:
            rate = (
                breakdown[0]["rate"]
                if breakdown
                else float(inv.vat_percent or 21)
            )
            ...
            rows.append((
                "normal",
                f"PVM {formatted_rate}%:",
                f"{_fmt(vat_total)} {sym}",
            ))
    ...
    for row_type, label, value in rows:
        if row_type == "indent":
            summary_data.append([
                Paragraph(
                    f"&nbsp;&nbsp;&nbsp;{label}",
                    s["total_indent_label"],
                ),
                Paragraph(
                    value,
                    s["total_indent_value"],
                ),
            ])
        elif row_type == "grand":
            summary_data.append([
...
def save_invoice_pdf(invoice):
    ...
    filename = f"saskaita_{invoice.document_series}{invoice.document_number}.pdf"
