...
Extract into "fees" every separately stated charge that is not a product/service line: delivery (pristatymas, siuntimas, kurjeris), packaging (pakavimas), deposit (tara, užstatas, depozitas, depozitinis, grąžinamas užstatas), payment/handling/administration fees (mokėjimo, apdorojimo, administravimo mokestis). For each: "type" (pristatymas | pakavimas | tara | uzstatas | mokestis), "amount" (exactly as printed) and "includes_vat" (true if the printed amount is VAT-inclusive, false if it is stated without VAT, e.g. "Pristatymas be PVM"). If a deposit is summarised on one line in the totals (e.g. "Depozitas 4,00"), report that single total, not the individual deposit rows. Do NOT put discounts or rounding here, and do not guess a VAT rate for fees. If there are no such charges, omit "fees".
